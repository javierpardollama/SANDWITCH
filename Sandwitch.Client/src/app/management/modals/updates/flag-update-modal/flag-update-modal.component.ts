...
export class FlagUpdateModalComponent implements OnInit {
  // DI
  private flagService = inject(FlagService);
  private formBuilder = inject(FormBuilder);
  dialogRef = inject<MatDialogRef<FlagUpdateModalComponent>>(MatDialogRef);
  private matSnackBar = inject(MatSnackBar);
  data = inject<ViewFlag>(MAT_DIALOG_DATA);
  ...
  // Form Actions
  async onSubmit(viewModel: UpdateFlag): Promise<void> {
    let bandera = await this.flagService.UpdateFlag(viewModel);
  ...
  async onDelete(viewModel: UpdateFlag): Promise<void> {
    await this.flagService.RemoveFlagById(viewModel.Id);
