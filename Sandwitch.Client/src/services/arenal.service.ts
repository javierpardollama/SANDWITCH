...
export class ArenalService extends BaseService {
    ...
    public UpdateArenal(viewModel: UpdateArenal): Promise<ViewArenal> {
        return firstValueFrom(this.httpClient.put<ViewArenal>(`${environment.Service}api/arenal/updatearenal`, viewModel)
            .pipe(catchError(this.HandleError<ViewArenal>('UpdateArenal', undefined))));
    }

    public FindAllArenal(): Promise<ViewArenal[]> {
        return firstValueFrom(this.httpClient.get<ViewArenal[]>(`${environment.Service}api/arenal/findallarenal`)
    ...
    public FindPaginatedArenal(viewModel: FilterPage): Promise<ViewPage<ViewArenal>> {
        return firstValueFrom(this.httpClient.post<ViewPage<ViewArenal>>(`${environment.Service}api/arenal/findpaginatedarenal`, viewModel)
    ...
    public FindAllArenalByPoblacionId(id: number): Promise<ViewArenal[]> {
        return firstValueFrom(this.httpClient.get<ViewArenal[]>(`${environment.Service}api/arenal/findallarenalbypoblacionid/` + id)
    ...
    public AddArenal(viewModel: AddArenal): Promise<ViewArenal> {
        return firstValueFrom(this.httpClient.post<ViewArenal>(`${environment.Service}api/arenal/addarenal`, viewModel)
            .pipe(catchError(this.HandleError<ViewArenal>('AddArenal', undefined))));
    }

    public RemoveArenalById(id: number) {
        return this.httpClient.delete<any>(`${environment.Service}api/arenal/removearenalbyid/` + id)
