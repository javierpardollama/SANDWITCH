...
public class FlagController
{
    ...
    public async Task<IList<ViewCatalog>> FindAllFlag()
    {
        ...
        var Flags = await response.Content.ReadFromJsonAsync<List<ViewCatalog>>();

        return Flags;
    }

    [Benchmark]
    public async Task<ViewPage<ViewFlag>> FindPaginatedFlag()
    {
        Client.DefaultRequestHeaders.Authorization = Client.DefaultRequestHeaders.Authorization =
            new AuthenticationHeaderValue("Basic",
                Convert.ToBase64String(Encoding.UTF8.GetBytes("Peach" + ":" + "T/R4J6eyvNG<6ne!")));
        ...
        response.EnsureSuccessStatusCode();
        var Flags = await response.Content.ReadFromJsonAsync<ViewPage<ViewFlag>>();
