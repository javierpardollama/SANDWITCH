...
{
    private static readonly HttpClient Client = new() { BaseAddress = new Uri("https://localhost:8081/api/v2/wind/") };
    ...
    [Benchmark]
    public async Task<ViewPage<ViewWind>> FindPaginatedWind()
    {
        Client.DefaultRequestHeaders.Authorization = Client.DefaultRequestHeaders.Authorization =
            new AuthenticationHeaderValue("Basic",
                Convert.ToBase64String(Encoding.UTF8.GetBytes("Peach" + ":" + "T/R4J6eyvNG<6ne!")));
