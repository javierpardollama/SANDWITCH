...
public class ViewArenal : ViewBase
{
    ...
    public virtual ViewHistorico LastHistorico =>
        Historicos?.AsQueryable().OrderBy(x => x.LastModified.Date).LastOrDefault();
    ...
    public virtual IList<ViewPoblacion> Poblaciones =>
        ArenalPoblaciones?.AsQueryable().Select(x => x.Poblacion).ToList();
}